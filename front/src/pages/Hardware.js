...
function Hardware() {
      ...
      if (data.type === 'sensor_data' && data.deviceId) {
        ...
        if (error) {
          ...
          setHardwareAlerts(prev => {
            const existingIndex = prev.findIndex(alert => 
          ...
          setHardwareAlerts(prev => prev.filter(alert => alert.deviceId !== data.deviceId))
        }
      }
    }
  ...
  const tryAutoConnect = async () => {
    ...
    try {
      ...
      if (portInfoData) {
        // USB 포트인지 확인
        if (portInfoData.usbVendorId !== undefined && portInfoData.usbProductId !== undefined) {
          return port
        }
      }
  ...
  const connectToPort = async (port, isAutoConnect = false) => {
    ...
    try {
      ...
      if (portInfoData) {
        ...
        // 블루투스 포트 감지 시 경고
        if (portInfoData.usbVendorId === undefined && portInfoData.usbProductId === undefined) {
          appendLog("⚠ 경고: USB 포트가 아닐 수 있습니다. 블루투스 포트는 작동하지 않습니다.")
          appendLog("⚠ 블루투스 포트를 선택하셨다면, USB 포트를 다시 선택해주세요.")
        } else {
          appendLog(`✓ USB 포트 확인됨 (Vendor: 0x${portInfoData.usbVendorId?.toString(16)}, Product: 0x${portInfoData.usbProductId?.toString(16)})`)
        }
      }
      ...
      setSearchStatus({ type: 'success', message: isAutoConnect ? 'USB 자동 연결 완료! 이제 WiFi 정보를 입력하세요.' : 'USB 연결 완료! 이제 WiFi 정보를 입력하세요.' })
  ...
  const handleSearchDevices = async () => {
    ...
    try {
      ...
      if (!navigator.serial) {
        setSearchStatus({ 
          type: 'error', 
          message: '이 브라우저는 USB 시리얼 포트 접근을 지원하지 않습니다. Chrome, Edge, Opera 등의 최신 브라우저를 사용해주세요.' 
        })
        setIsSearching(false)
        return
      }
      ...
      if (!port) {
        setSearchStatus({ type: 'info', message: 'USB 포트를 선택해주세요...' })
      ...
      if (error.name === 'NotFoundError') {
        ...
        setSearchStatus({ 
          type: 'error', 
          message: 'USB 시리얼 포트 접근 권한이 필요합니다. 브라우저 설정에서 권한을 확인해주세요.' 
        })
      } else if (error.message?.includes("blocklist") || error.message?.includes("blocked")) {
        setSearchStatus({ 
          type: 'error', 
          message: '선택한 포트가 차단되었습니다. 블루투스 포트가 아닌 USB 포트를 선택해주세요.' 
        })
      ...
      try {
        ...
        if (value) {
          ...
          lines.forEach(line => {
            ...
            // 부팅 로그 필터링
            if (filterBootLog) {
              const bootLogMatch = trimmedLine.match(/(esp_image:|segment \d+:|paddr=|vaddr=|I \(\d+\)|W \(\d+\)|E \(\d+\))/)
              if (bootLogMatch) {
                ...
                if (isBootLog(trimmedLine) && !dataPatterns.some(p => p.test(trimmedLine))) {
                  return
                }
            ...
            const macMatch = trimmedLine.match(/([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})/i)
            if (macMatch) {
              ...
              if (tempWifiInfo && (!detectedMacAddress || detectedMacAddress !== macAddress)) {
                try {
                  const wifiInfo = JSON.parse(tempWifiInfo)
                  // MAC 주소로 WiFi 정보 저장
                  localStorage.setItem(`hub_wifi_${macAddress}`, JSON.stringify(wifiInfo))
                  // 임시 저장 삭제
                  localStorage.removeItem('hub_wifi_temp')
                  console.log(`[Hub Register] WiFi 정보를 MAC 주소(${macAddress})로 저장했습니다.`)
                } catch (e) {
  ...
  const checkForDeviceSearchResults = (line) => {
    ...
    if (line.includes('searchDevice') || line.includes('Search') || line.includes('검색') || 
        line.includes('Device search') || line.includes('디바이스 검색')) {
    ...
    if (macMatch && !line.includes('wifi') && !line.includes('HTTP') && !line.includes('ESP') && !line.includes('GATTS')) {
      const macAddress = macMatch[0].replace(/-/g, ':').toUpperCase()
      const exists = scannedDevices.some(device => device.macAddress === macAddress)
    ...
    // 검색 완료 신호 확인
    if (line.includes('Search complete') || line.includes('검색 완료') || 
        line.includes('Found') || line.includes('found')) {
      setIsScanning(false)
  ...
  const checkForRegistrationComplete = (line) => {
    ...
    if (line.includes('HTTP POST 성공') || line.includes('HTTP_POST 성공') || 
        line.includes('Status: 200') || line.includes('등록 완료') ||
        line.includes('"success":true') || line.includes('connected : success')) {
      // JSON 응답에서 success 확인
      if (line.includes('"success":true') || line.includes('등록 완료') || line.includes('connected : success')) {
        // MAC 주소가 없으면 추출 시도
  ...
  const handleRegisterHub = async () => {
    ...
    try {
      ...
      appendLog(`PC → WiFi 정보 전송: ID=${wifiId}, Password=${wifiPassword ? "***" : "(없음)"}`)
      ...
      if (detectedMacAddress) {
        ...
        // WiFi 정보를 MAC 주소로 저장
        localStorage.setItem(`hub_wifi_${detectedMacAddress}`, JSON.stringify(wifiInfo))
        // 임시 저장 삭제
        localStorage.removeItem('hub_wifi_temp')
      }
        ...
        setSearchStatus({ 
        type: 'info', 
        message: 'WiFi 설정이 전송되었습니다. 허브가 WiFi에 연결되고 등록을 완료할 때까지 기다려주세요...' 
        })
    } catch (err) {
      appendLog("전송 실패: " + err)
        setSearchStatus({ 
  ...
  const handleFinalRegister = async () => {
    console.log('[Device Register] 시작 - devicesToRegister:', devicesToRegister)
    ...
    try {
      ...
      const registrationResults = await Promise.allSettled(
        devicesWithNames.map(async ([deviceId, data]) => {
          console.log(`[Device Register] 디바이스 등록 시도 - ID: ${deviceId}, Name: ${data.name.trim()}`)
          const device = scannedDevices.find(d => d.id === deviceId)
          if (!device || !device.macAddress) {
            throw new Error(`디바이스 정보를 찾을 수 없습니다: ${deviceId}`)
          }

          console.log(`[Device Register] 디바이스 MAC 주소: ${device.macAddress}`)
          ...
          return { deviceId, name: data.name.trim(), macAddress: device.macAddress }
        })
      )
      ...
      // 성공/실패 결과 확인
      const successful = registrationResults.filter(r => r.status === 'fulfilled').length
      ...
      handleCloseDeviceRegister()
        ...
        const errorMessages = registrationResults
          ...
          .map(r => {
            ...
            return r.reason?.message || r.reason?.toString() || '알 수 없는 오류'
          })
          .join(', ')
  ...
  useEffect(() => {
    if (!isErrorSimulationActive) {
      ...
      setSimulatedErrors({});
      // 시뮬레이션 오류 알림 제거
      setHardwareAlerts(prev => prev.filter(alert => !alert.id?.includes('simulated')));
      return;
    }
    ...
    const triggerRandomError = () => {
      ...
      if (Math.random() < 0.3) {
        ...
        errorDurationRefs.current[deviceId] = setTimeout(() => {
          ...
          setHardwareAlerts(prev => prev.filter(alert => 
            !(alert.deviceId === deviceId && alert.id?.includes('simulated'))
          ));
                  ...
                  <div className="hub-info">
                    ...
                    <div className="hub-details">
                      ...
                      <div className="detail-item">
                        ...
                        <span className="device-count">{hub.connectedDevices || 0}개</span>
                      </div>
                      <div className="detail-item">
                        <span className="label">상태:</span>
                        <div className="hub-status-container">
                          <span className={hubStatuses[hub.address] ? 'status-online' : 'status-offline'}>
                            {hubStatuses[hub.address] ? '🟢 온라인' : '🔴 오프라인'}
                        </span>
                          {hubActivityProgress[hub.address] && (
                            <div className="hub-activity-progress">
                              <div className="progress-bar-container">
                                <div 
                                  className={`progress-bar ${hubActivityProgress[hub.address].isActive ? 'active' : ''}`}
                                  style={{ width: `${hubActivityProgress[hub.address].progress}%` }}
                                />
                              </div>
                            </div>
                          )}
          ...
          <div className="device-section">
            ...
            <div className="device-list">
              {devices.map(device => {
                ...
                const deviceError = simulatedError || (deviceData ? detectHardwareError(deviceData.heartRate) : null)
                return (
        ...
        <div className="modal-overlay">
          <div className="modal-content hub-register-modal">
            ...
            <div className="modal-body">
              <div className="hub-register-content">
                ...
                <div className="registration-steps">
                  <div className={`step-indicator ${registrationStep >= 1 ? 'active' : ''} ${registrationStep > 1 ? 'completed' : ''}`}>
                    <div className="step-number">1</div>
                    <div className="step-label">USB 연결</div>
                  </div>
                  <div className={`step-indicator ${registrationStep >= 2 ? 'active' : ''} ${isRegistered ? 'completed' : ''}`}>
                ...
                <p className="hub-register-instruction">
                      USB 선으로 허브를 연결한 후, 아래 버튼을 클릭하여 USB 포트를 선택하세요.
                </p>
                ...
                <div className="search-section">
                    ...
                    >
                      {isSearching ? '연결 중...' : isReading ? '수신 중...' : 'USB 포트 연결'}
                    </button>
                  
                  {searchStatus.type && (
                    <div className={`search-status-message ${searchStatus.type}`}>
                      ...
                      {portInfo && (
                        <div className="port-info">
                          <div className="port-info-item">
                            ...
                            <span className="port-info-value">0x{portInfo.usbVendorId?.toString(16).toUpperCase()}</span>
                  </div>
                          <div className="port-info-item">
                            <span className="port-info-label">Product ID:</span>
                            <span className="port-info-value">0x{portInfo.usbProductId?.toString(16).toUpperCase()}</span>
                          </div>
                      </div>
                    )}
                  </div>
                ...
                {registrationStep === 2 && (
                  <>
                    <p className="hub-register-instruction">
                      WiFi 정보를 입력하고 전송 버튼을 클릭하세요. 허브가 WiFi에 연결되면 자동으로 등록됩니다.
                    </p>
                  ...
                  <div className="wifi-form-section">
                    ...
                    <div className="form-group">
                        <label htmlFor="wifi-password">WiFi 비밀번호 (선택사항)</label>
                      <input
                        id="wifi-password"
                        type="password"
                        value={wifiPassword}
                    ...
                    {detectedMacAddress && (
                      <div className="detected-mac">
                        <span className="detected-mac-label">감지된 MAC 주소:</span>
                        <span className="detected-mac-value">{detectedMacAddress}</span>
                      </div>
                    )}
            ...
            <div className="modal-footer">
              <button onClick={handleCloseHubRegister} className="btn-secondary">
                {isRegistered ? '닫기' : '취소'}
              </button>
            </div>
          </div>
